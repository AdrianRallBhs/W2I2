...
exports.getOutdatedPackages = exports.findALLCSPROJmodules = exports.getDotnetSources = exports.runRepoInfo = void 0;
...
async function getAllPackages() {
    try {
        const { stdout } = await (0, child_process_1.exec)('npm ls --json --depth=0 --parseable');
...
// ========================================
async function listNpmRegistries() {
...
async function runRepoInfo() {
    ...
    const dotNetProjects = await findALLCSPROJmodules();
...
try {
    ...
    (0, child_process_2.execSync)(`git clone ${repositoryUrl}`);
    process.chdir(repositoryDir);
    (0, child_process_2.execSync)(`git checkout ${featureBranchName}`, { shell: "/bin/bash" });
    (0, child_process_2.execSync)(`git config --global user.email ${email}`);
    (0, child_process_2.execSync)(`git config --global user.name \"${name}\"`);
    ...
    (0, child_process_2.execSync)(`git add .`);
    (0, child_process_2.execSync)(`git commit -m "Add output to output.json"`);
    (0, child_process_2.execSync)(`git push origin ${featureBranchName}`);
}
catch (error) {
    console.error(error);
}
//=====================================================
async function getDotnetSources() {
    return new Promise((resolve, reject) => {
...
// }
//======================funktioniert =============================
async function getOutdatedPackages(projectList, sourceList) {
...
//   return allPackages;
// }
async function getAllNuGetPackages(projectList, sourceList) {
    const allPackages = [];
    for (const project of projectList) {
        for (const source of sourceList) {
            const output = child_process.execSync(`dotnet list ${project} package --source ${source}`);
